...
def _secret_text(key: str) -> str | None:
    try:
...
def load_portfolio(path: Path = PORTFOLIO_PATH) -> pd.DataFrame:
    private_csv = _secret_text("portfolio_csv")
    portfolio = pd.read_csv(StringIO(private_csv)) if private_csv else pd.read_csv(path)
...
def load_consistency_snapshot(source: object | None = None) -> pd.DataFrame:
    ...
    if source is None:
        private_csv = _secret_text("score_snapshot_csv")
        if private_csv:
            snapshot = pd.read_csv(StringIO(private_csv))
        elif DEFAULT_SCORE_PATH.exists():
...
def main() -> None:
    ...
    try:
        ...
        st.info("Add a portfolio_csv secret in Streamlit Cloud, or place portfolio.csv beside this app when running locally. The score snapshot is optional.")
        return
