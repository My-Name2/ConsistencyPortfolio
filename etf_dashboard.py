...
def download_prices(tickers: tuple[str, ...], start: date, end: date, adjusted: bool) -> pd.DataFrame:
    ...
    missing = [ticker for ticker in requested if ticker not in close or close[ticker].notna().sum() == 0]
...
def main() -> None:
    ...
    if missing:
        st.warning("Yahoo Finance did not return history for: " + ", ".join(missing) + ". Those sleeves are held flat until data is available; refresh later to retry.")
