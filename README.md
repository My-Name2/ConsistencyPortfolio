...
The original live-session tracker remains available in `app.py`. It now reads the same 50-stock `portfolio.csv` by default:
...
There is also a second app for option-contract comparison:

```powershell
streamlit run options_lambda_app.py
```

It:

- pulls an option chain for a ticker with `yfinance`
- lets you filter expirations, strikes, and calls/puts
- compares two contracts side by side
- estimates delta, gamma, theta, and lambda from implied volatility
- shows a simple exposure-versus-decay heuristic for comparing different expirations

### TOS RTD mode

On Windows, the options app can use `TOS RTD via Excel` as an optional live quote source. It requires Thinkorswim running, desktop Excel, and `pywin32` from `requirements.txt`. Select the source in the sidebar; the app samples TOS `MARK`, bid/ask, IV, delta, gamma, and theta on each Streamlit rerun. Use `Full fetched chain` if you want the lambda calendar to use the TOS snapshot for the whole yfinance-returned chain.

TOS RTD is an Excel/COM bridge, not a direct Python API. The official setup and field list are documented by [thinkorswim RTD](https://toslc.thinkorswim.com/center/howToTos/thinkManual/Miscellaneous/Real-Time-Data-%28RTD%29-Function-for-thinkorswim.html). If TOS or Excel is unavailable, the app explicitly falls back to yfinance.
